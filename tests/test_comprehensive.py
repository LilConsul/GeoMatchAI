...
async def test_single_configuration(
    ...
    try:
        ...
        results.update({
            ...
            'time_inference_s': t_preprocess + t_feature_extract + t_verify,  # Without gallery build
            'error': None
        })
...
def save_results_to_csv(all_results: List[Dict], output_dir: Path):
    ...
    fieldnames = [
        ...
        'image_name', 'is_wawel', 'preprocessing',
        'similarity_score', 'is_verified', 'expected_verified', 'is_correct',
        'gallery_size', 'threshold',
        'time_gallery_build_s', 'time_preprocess_s', 'time_feature_extract_s',
        'time_verify_s', 'time_inference_s', 'time_total_s',
        'error'
    ]
    ...
    fieldnames = [
        ...
        'avg_wawel_score', 'avg_unrelated_score', 'discrimination_gap',
        'accuracy', 'correct_count', 'total_count',
        'avg_inference_time_s', 'min_inference_time_s', 'max_inference_time_s'
    ]
    ...
    with open(csv_path, 'w', newline='', encoding='utf-8') as f:
        ...
        for stats in model_stats.values():
            wawel_avg = sum(stats['wawel_scores']) / len(stats['wawel_scores']) if stats['wawel_scores'] else 0
            unrelated_avg = sum(stats['unrelated_scores']) / len(stats['unrelated_scores']) if stats['unrelated_scores'] else 0
            ...
            row = {
                ...
                'correct_count': stats['correct_count'],
                'total_count': stats['total_count'],
                'avg_inference_time_s': sum(stats['time_inference']) / len(stats['time_inference']) if stats['time_inference'] else 0,
                'min_inference_time_s': min(stats['time_inference']) if stats['time_inference'] else 0,
                'max_inference_time_s': max(stats['time_inference']) if stats['time_inference'] else 0,
            }
            writer.writerow(row)
    ...
    for model_type, model_variant, model_name in MODEL_CONFIGS:
        ...
        for img_path, img_name, is_wawel in test_images:
            for use_preprocessing in [True, False]:
                ...
                print(f"  [{test_count}/{total_tests}] {img_name} ({prep_str} preprocessing)...", end=" ")

                result = await test_single_configuration(
                    model_type, model_variant, model_name,
                    gallery_images, img_path, img_name, is_wawel,
                    use_preprocessing, device
                )
                ...
                if result['error']:
                    ...
                    print(f"{status} Score: {result['similarity_score']:.4f}, Time: {result['time_inference_s']:.3f}s")
