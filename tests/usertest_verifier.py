...
async def test_single_model(
    ...
    for img_type, use_prep, image in test_cases:
        ...
        results[key] = {"score": score, "verified": is_verified}

    # Calculate derived metrics
    results["gap_with_prep"] = (
        results["wawel_with_prep"]["score"] - results["unrelated_with_prep"]["score"]
    )
    results["gap_no_prep"] = (
        results["wawel_no_prep"]["score"] - results["unrelated_no_prep"]["score"]
    )
    results["prep_impact_wawel"] = (
        results["wawel_no_prep"]["score"] - results["wawel_with_prep"]["score"]
    )
    results["prep_impact_unrelated"] = (
        results["unrelated_no_prep"]["score"] - results["unrelated_with_prep"]["score"]
    )
...
def print_preprocessing_comparison(all_results: dict[str, dict], display_names: dict[str, str]):
    ...
    for model_key, results in all_results.items():
        ...
        print(
            f"{model_name:<30} {'WITH prep':<15} {wawel_prep:<10.4f} {gap_prep:<10.4f} {impact_wawel:+.4f} ({impact_wawel / wawel_no_prep * 100:+.1f}%)"
            if wawel_no_prep > 0
            else f"{model_name:<30} {'WITH prep':<15} {wawel_prep:<10.4f} {gap_prep:<10.4f}"
        )
...
def print_detailed_results(all_results: dict[str, dict], display_names: dict[str, str]):
    ...
    for model_key, results in all_results.items():
        ...
        print(
            f"      Gap:            {results['gap_no_prep']:.4f} ({results['gap_no_prep'] * 100:.1f}%)"
        )
        print()
...
def print_recommendations(all_results: dict[str, dict], display_names: dict[str, str]):
    ...
    if best_model_key and all_results.get(best_model_key):
        ...
        impact_pct = (
            (impact / results["wawel_no_prep"]["score"] * 100)
            if results["wawel_no_prep"]["score"] > 0
            else 0
        )
