...
def load_data():
    ...
    if missing_files:
        ...
        print("="*100 + "\n")
        raise FileNotFoundError(f"Missing CSV files in {CSV_DIR}. Please run test_comprehensive.py first!")
...
def plot_model_comparison_heatmap(df_summary):
    ...
    ax.set_title('Model Performance Heatmap (WITH Preprocessing)\nHigher scores = Better for Wawel, Lower = Better for Unrelated',
...
def plot_discrimination_gap(df_by_model):
    ...
    bars2 = ax.bar(x_pos + width/2, without_gaps, width, label='WITHOUT Preprocessing', color='#3498db')
...
def plot_preprocessing_impact(df_by_model):
    ...
    for model in models:
        ...
        with_row = df_model[df_model['preprocessing'] == True].iloc[0] if len(df_model[df_model['preprocessing'] == True]) > 0 else None
        without_row = df_model[df_model['preprocessing'] == False].iloc[0] if len(df_model[df_model['preprocessing'] == False]) > 0 else None
    ...
    # Plot 1: Wawel scores
    bars1 = ax1.bar(x - width/2, wawel_with, width, label='WITH Preprocessing', color='#2ecc71', alpha=0.8)
    bars2 = ax1.bar(x + width/2, wawel_without, width, label='WITHOUT Preprocessing', color='#95a5a6', alpha=0.8)

    ax1.set_xlabel('Model', fontsize=11, fontweight='bold')
    ax1.set_ylabel('Average Similarity Score', fontsize=11, fontweight='bold')
    ax1.set_title('Wawel Images: Preprocessing Impact\n(Higher = Better)', fontsize=12, fontweight='bold')
    ax1.set_xticks(x)
    ...
    bars3 = ax2.bar(x - width/2, unrelated_with, width, label='WITH Preprocessing', color='#e74c3c', alpha=0.8)
    bars4 = ax2.bar(x + width/2, unrelated_without, width, label='WITHOUT Preprocessing', color='#c0392b', alpha=0.8)

    ax2.set_xlabel('Model', fontsize=11, fontweight='bold')
    ax2.set_ylabel('Average Similarity Score', fontsize=11, fontweight='bold')
    ax2.set_title('Unrelated Images: Preprocessing Impact\n(Lower = Better)', fontsize=12, fontweight='bold')
    ax2.set_xticks(x)
...
def plot_timing_comparison(df_by_model):
    ...
    bars2 = ax.bar(x + width/2, without_times, width, label='WITHOUT Preprocessing', color='#95a5a6')
...
def plot_accuracy_comparison(df_by_model):
    ...
    bars2 = ax.bar(x + width/2, without_acc, width, label='WITHOUT Preprocessing', color='#3498db')
...
def plot_score_distribution(df_summary):
    ...
    for idx, model in enumerate(models[:4]):  # Limit to 4 models
        ...
        # Plot histograms
        ax.hist(wawel_scores, bins=20, alpha=0.6, label='Wawel (Should Pass)', color='#2ecc71', edgecolor='black')
        ax.hist(unrelated_scores, bins=20, alpha=0.6, label='Unrelated (Should Fail)', color='#e74c3c', edgecolor='black')
    ...
    plt.suptitle('Similarity Score Distributions by Model\n(WITH Preprocessing)',
...
def create_summary_report(data):
    ...
    with open(report_path, 'w', encoding='utf-8') as f:
        ...
        f.write(f"Discrimination Gap: {best_model['discrimination_gap']:.4f} ({best_model['discrimination_gap']*100:.1f}%)\n")
        ...
        f.write(f"{'Model':<30} {'Gap':<10} {'Wawel':<10} {'Unrel':<10} {'Acc':<8} {'Time(ms)':<10}\n")
        f.write("-"*100 + "\n")

        df_sorted = df_with_prep.sort_values('discrimination_gap', ascending=False)
        for _, row in df_sorted.iterrows():
            f.write(f"{row['model_name']:<30} "
        ...
        f.write(f"Recommended Production Config:\n")
        f.write(f"  Model: {best_model['model_name']}\n")
        f.write(f"  Threshold: 0.65\n")
        f.write(f"  Preprocessing: ENABLE for user queries\n")
